...
from app.schemas.car import CarSchema, PageResponse


class PersonRepository:

    @staticmethod
    async def create(create_form: CarSchema):
        """ create person data """
        db.add(Car(
            title=create_form.title,
            content=create_form.content,
            date=create_form.birth_date,
            city=create_form.city
        ))
        await commit_rollback()

    @staticmethod
    async def get_by_id(car_id: int):
        """ retrieve car data by id """
        query = select(Car).where(Car.id == car_id)
        return (await db_session.execute(query)).scalar_one_or_none()

    @staticmethod
    async def update(car_id: int, update_form: CarSchema):
        """ update car data """
    ...
    async def get_all(
        ...
        query = select(from_obj=Car, columns="*")

        # select columns dynamically
        if columns is not None and columns != "all":
            # we need column format data like this --> [column(id),column(name),column(sex)...]

            query = select(from_obj=Car, columns=convert_columns(columns))
